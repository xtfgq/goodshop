	...
	<scroll-view class="bg-white " >
		<view>
			...
			<view style="text-align: center; width: 100%; ">
				<image  src="/static/img/home/code.png" style="margin: 0 auto;" class="tips_code" />
			</view>
			<view style="text-align: center; width: 100%; ">
				<view  style="margin: 0 auto;">
					<text class="text-black text-bold text-xl">最新成交价：</text>
					<text class="text-price">500</text>
				</view>
			</view>
			<view style="text-align: center; width: 100%; margin-top: 20rpx">
				<view  style="margin: 0 auto;">
					<text class="text-gray text-bold text-sm">查看全部报价 ></text>
				</view>
			</view>
			<view class="flex">
				<view class="flex-sub  padding-sm margin-xl radius" style="border:2rpx solid #EEEFF3;">
					<image  src="/static/img/home/finish.png" style="height: 24rpx; width: 24rpx;margin-left: 72rpx " />
					<text style="margin-left: 15rpx " class="text-sm">我有</text>
				</view>
				<view class="flex-sub  padding-sm margin-xl radius" style="border:2rpx solid #EEEFF3;">
					<image  src="/static/img/home/need.png" style="height: 24rpx; width: 24rpx;margin-left: 72rpx " />
					<text style="margin-left: 15rpx " class="text-sm">想要</text>
				</view>

			</view>
			<view class="flex " style="margin-top: -35rpx">
				<PersonNum :list="hasData"></PersonNum>
				<PersonNum :list="needData"></PersonNum>
			</view>
			<view style="text-align: center; width: 100%; ">
				<image src="/static/img/home/notice.png" style="width: 681rpx;height:94rpx" mode="scaleToFill"></image>
			</view>

			<view class="cu-bar  bg-white">
				<view class="action">
					<text class="name padding-left-sm">购买记录：（2324）</text>
				</view>
				<view class="action padding-right">
					<xfl-select
							...
							:list="totalArray"
							:clearable="false"
							:style_Container="'height: 45rpx; font-size: 14px;width:168rpx'"
							:initValue = "totalArray[0]"
							@change = "change($event,index)"
					>
					</xfl-select>
				</view>
			</view>
			<view v-for="(order,index) in transItems" :key="index"
				  class="order_item bg-white grid col-3  text-lg  text-bold text-center">
				<view class="text-black text-df text-center margin-top-sm" >{{getTimeFormat(order.time)}}</view>
				<view class="text-center margin-tb-sm"><text class="text-df">{{order.type}}</text><view class="cu-btn round line-black text-sm">{{getBtnType(order.ordertype)}}</view></view>

				<view class="text-black text-df text-price margin-tb-sm">{{order.price}}</view>
			</view>
			<view class="cu-bar  bg-white " >
				<view class="action">
					<text class="text-black text-bold">讨论（543）</text>
				</view>
				<view class="action">
					<button class="cu-btn block line-black lg">
						<text class="cuIcon-add text-black "></text><text class="text-black text-sm">添加</text></button>
				</view>
			</view>
			...
			<view class="moments__post" v-for="(post,index) in posts" :key="index" :id="'post-'+index">
				...
				<view class="flex bg-white" style="position:fixed; bottom:0;height: 128rpx;width: 100%;">
					<view class="flex  bg-black  justify-around bottom_block" @click="showSell()">
						<view  style="margin-top:10upx;display:flex;flex-direction:column;align-items: center;font-size: 22upx;" >
							<view class="text-white">最高购买价</view>
							<view class="text-price text-white">343</view>
						</view>
						<view class="itemline" style="border-right: 1rpx solid #FFFFFF;"></view>
						<view class="text-white " style="margin-right:40rpx;font-size: 28upx;">出售</view>
					</view>
					<view class="flex  bg-yellow  justify-around bottom_block" @click="showBuy()">
						<view  style="margin-top:10upx;display:flex;flex-direction:column;align-items: center;font-size: 22upx;" >
							<view class="text-black">最低售价</view>
							<view class="text-price text-black">313</view>
						</view>
						<view class="itemline " style="border-right: 1rpx solid #000000;"></view>
						<view class=" text-black" style="margin-right:40rpx;font-size: 28upx;">购买</view>
					</view>
				</view>
...
<script>
	...
	import PersonNum from "@/components/person_num.vue"
	...
	export default {
		...
		onShow(){
			...
			for(let i=0;i<5;i++){
				...
				this.needData.push(obj);

			}
	        for(let i=0;i<15;i++){
				let price = 232+i.toString();
				let timeStr='';
				if(i%2==0){
					timeStr='2020-05-06 08:20:00';
				}else{
					timeStr='2020-05-01 10:05:00';
				}
				let date =this.parse(timeStr).getTime();
				let obj ={
					price:price,
					time:date,
					type:"08*2 2020",
					ordertype:Number(i%2==0)
				}
				this.transItems.push(obj);
		...
		methods: {
			...
			getTimeFormat(val){
				console.log("time=="+JSON.stringify(val));
				let data=new Date(val);
				return util.dateUtils.getDateDiff(data.getTime());
			},
			getBtnType(val){
				if(val==0){
					return '团购';
